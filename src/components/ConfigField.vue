...
function submitTag(submit: (value: string) => void) {
...
<template>
  <div class="config-field" :class="{ 'config-field--switch': field.type === 'switch', 'config-field--wide': field.type === 'tags' || field.type === 'textarea' }">
    ...
    <NDynamicTags v-else-if="field.type === 'tags'" :value="tagValue" @update:value="setTags"><template #input="{ submit, deactivate }"><NInput v-model:value="tagDraft" size="small" :placeholder="field.placeholder" @blur="deactivate" @keyup.enter="submitTag(submit)" /></template></NDynamicTags>

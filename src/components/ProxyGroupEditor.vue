...
const rawGroups = computed(() => Array.isArray(store.get('proxy-groups')) ? store.get('proxy-groups') as unknown[] : [])
const groupItems = computed(() => rawGroups.value.flatMap((value, index) => value && typeof value === 'object' && !Array.isArray(value)
  ? [{ value: value as Record<string, unknown>, index }]
  : []))
const groups = computed(() => groupItems.value.map((item) => item.value))
...
const memberOptions = computed(() => {
  const otherGroups = groupItems.value.map((item) => ({ index: item.index, name: String(item.value.name || '') })).filter((item) => item.name && item.index !== editingIndex.value).map((item) => ({ label: item.name, value: item.name }))
...
const shown = computed(() => groupItems.value.map((item, position) => ({ ...item, position })).filter(({ value }) => {
...
function openEdit(index: number) {
  const value = rawGroups.value[index]
...
function save() {
  ...
  if (groupItems.value.some((item) => item.index !== editingIndex.value && item.value.name === name)) return message.error(`代理组“${name}”已经存在`)
  if (selectedMembers.value.includes(name)) return message.error('代理组不能引用自身')
  const next = clone(rawGroups.value)
...
function remove(index: number) { store.setRoot('proxy-groups', rawGroups.value.filter((_, itemIndex) => itemIndex !== index)) }
function move(index: number, offset: number) {
  const position = groupItems.value.findIndex((item) => item.index === index)
  const target = groupItems.value[position + offset]?.index

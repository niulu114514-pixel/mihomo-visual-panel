...
import { ArrowDown, ArrowUp, Layers3, Pencil, Plus, Trash2 } from '@lucide/vue'
...
const typeOptions = ['selector', 'urltest', 'direct', 'block', 'socks', 'http', 'shadowsocks', 'vmess', 'vless', 'trojan', 'hysteria2', 'tuic', 'anytls'].map((value) => ({ label: value, value }))
...
<template>
  <section class="singbox-module-panel">
    <header class="singbox-module-heading"><div><h2>出站与代理组</h2><p>普通节点、直连/拦截以及可嵌套的 selector、urltest 代理组统一管理。</p></div><div class="heading-actions"><NButton secondary @click="openNew('selector')"><template #icon><Layers3 :size="15" /></template>添加代理组</NButton><NButton type="primary" @click="openNew('direct')"><template #icon><Plus :size="15" /></template>添加出站</NButton></div></header>
